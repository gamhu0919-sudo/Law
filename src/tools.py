    ...
    try:
        ...
        for law in root.findall(".//law"):
            law_data = {
                ...
                "법령일련번호": law.findtext("법령일련번호", ""),  # 상세 조회에 필요한 MST 값
...
def _get_law_detail_impl(law_id: str, arguments: Optional[dict] = None) -> Dict:
    ...
    try:
        ...
        # 기본정보 섹션에서 올바른 태그명으로 추출
        info = root.find(".//기본정보")
        law_info = {
            "법령ID": (info.findtext("법령ID", "") if info is not None else root.findtext(".//법령ID", "")),
            "법령명": (info.findtext("법령명_한글", "") if info is not None else root.findtext(".//법령명_한글", "")),
            "법령구분": (info.findtext("법종구분", "") if info is not None else root.findtext(".//법종구분", "")),
            "소관부처": (info.findtext("소관부처", "") if info is not None else root.findtext(".//소관부처", "")),
            "공포일자": (info.findtext("공포일자", "") if info is not None else root.findtext(".//공포일자", "")),
            "시행일자": (info.findtext("시행일자", "") if info is not None else root.findtext(".//시행일자", "")),
            "제개정구분": (info.findtext("제개정구분", "") if info is not None else root.findtext(".//제개정구분", "")),
        }
        
        # 조문 정보 (실제 태그: 조문단위, 하위에 항/호/목 포함)
        articles = []
        for article in root.findall(".//조문단위"):
            조문번호_raw = article.findtext("조문번호", "")
            조문내용_raw = article.findtext("조문내용", "").strip()
            조문여부 = article.findtext("조문여부", "")
            ...
            # 조문번호 표시 (「제N조」 형식 정규화)
            try:
                조문번호_int = int(조문번호_raw)
                조문번호_display = f"제{조문번호_int}조"
            except (ValueError, TypeError):
                조문번호_display = 조문번호_raw if 조문번호_raw else ""
            
            조문제목 = article.findtext("조문제목", "").strip()
    ...
    try:
        ...
        for prec in root.findall(".//prec"):
            prec_data = {
                ...
                "선고일자": normalize_date(prec.findtext("선고일자", "")),
                "선고": prec.findtext("선고", ""),
                "법원명": prec.findtext("법원명", ""),
                "사건종류명": prec.findtext("사건종류명", ""),
                "판시사항": clean_html(prec.findtext("판시사항", "")),
                "판결요지": clean_html(prec.findtext("판결요지", ""))
            }
...
def get_precedent_detail(precedent_id: str, arguments: dict = None) -> Dict:
    ...
    try:
        ...
        prec_info = {
            ...
            "참조판례": clean_html(root.findtext(".//참조판례", "")),
            "판례내용": clean_html(root.findtext(".//판례내용", ""))
        }
    ...
    try:
        ...
        for rule in root.findall(".//admrul"):
            rule_data = {
                ...
                "행정규칙일련번호": rule.findtext("행정규칙일련번호", ""),
                "행정규칙명": rule.findtext("행정규칙명", ""),
                "행정규칙종류": rule.findtext("행정규칙종류", ""),
                "소관부처": rule.findtext("소관부처명", ""),
                "발령일자": rule.findtext("발령일자", ""),  # API는 발령일자 사용 (제정일자 없음)
                "시행일자": rule.findtext("시행일자", ""),
                "제개정구분": rule.findtext("제개정구분명", ""),
            }
