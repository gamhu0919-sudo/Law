...
def _find_nodes_by_local_name(root: ET.Element, local_names: List[str]) -> List[ET.Element]:
    """
    네임스페이스 유무와 관계없이 로컬 태그명 기준으로 노드를 찾습니다.
    """
    target_names = set(local_names)
    matched = []
...
def _get_law_detail_impl(law_id: str, arguments: Optional[dict] = None) -> Dict:
    ...
    try:
        ...
        # 조문 정보
        articles = []
        article_nodes = _find_nodes_by_local_name(root, ["조문", "조문단위"])
        seen_articles = set()
        for article in article_nodes:
            article_no = (
                article.findtext("조문번호", "")
                or article.findtext("조문번호한글", "")
            )
            article_title = article.findtext("조문제목", "")
            article_content = article.findtext("조문내용", "")

            # 응답 스키마에 따라 조문내용이 누락되는 경우가 있어 하위 텍스트를 fallback으로 사용
            if not article_content:
                article_content = "".join(article.itertext())

            article_content = _normalize_text(article_content)
            article_data = {
                "조문번호": _normalize_text(article_no),
                "조문제목": _normalize_text(article_title),
                "조문내용": article_content
            }

            # 비어있는 가짜 노드 제거
            if not any(article_data.values()):
                continue
            article_key = (
                article_data["조문번호"],
                article_data["조문제목"],
